...
#   Description:
#   ============
#   Web services or local-based routines to access FOSTA and UniProt FASTA
#   files.
#
#*************************************************************************
#
...
sub getIDFromAC {
    my ($ac, %opts) = @_;
    my $FASTAStr = GetFASTA($ac, %opts);
    return parseIDFromFASTAStr($FASTAStr); 
}

sub parseIDFromFASTAStr {
    my $FASTAStr = shift;
    my ($id) = $FASTAStr =~ /> \S+ \| \S+ \| (\S+)/xms;
    return $id;
}
...
sub GetFASTA
{
    ...
    if (! exists $opt{-local} && ! exists $opt{-remote}) {
        # Use config to determine how GetFASTA should be run
        require config;
        if($config::LocalSwissProt){
            ...
            return WEB::UNIPROT::GetFASTA($id);
        }
    }
    else {
        if ($opt{-local}) {
            croak "No implementation for running GetFASTA locally without a config module!";
        }
        else {
            return WEB::UNIPROT::GetFASTA($id);
        }
